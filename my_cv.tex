...
\cventry{Oct 2023 -- now}{Google LLC}{Eng Manager}{Persistent Disk}{}{
	\begin{itemize}
		\item Manager responsibilities \begin{itemize}
			\item Leading a team of \textbf{6-8 engineers}.
			\item Driving multiple promo cases, L3 $\rightarrow$ L4, L4 $\rightarrow$ L5, L5 $\rightarrow$ L6.
			\item I've ensured proper balance and fairness when proposing year-end ratings. Both higher and lower ratings were accepted by local TLs in the org and made sense to engineers when communicated to them.
			\item I have experience of dealing with \textbf{non-trivial performance cases} in the team.
			\item I use a rigorous approach when \textbf{hiring}, prioritizing finding the right talent for the team under pressure to close on the Hiring sooner.
			\item I'm \textbf{raising the bar} of our team's production oncall rotation by performing deep perf investigations and presenting them to large audiences. I've built processes to monitor the oncall load and reduce the noise in the rotation to keep the team focused on important issues while not being overloaded.
		\end{itemize}
		\item Product description \begin{itemize}
			\item Persistent Disk is a core Google Cloud product that provides attachable block devices (disks) for all Virtual Machines in Cloud.
 			\item Huge volume of traffic consisting of various high throughput and low latency customer workloads
